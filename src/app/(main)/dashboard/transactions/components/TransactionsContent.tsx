"use client";
import TitleContent from "@/ui/components/TitleContent";

export default function TransactionsContent(): React.ReactNode {
  return (
    <div className="p-6 space-y-6">
      <TitleContent
        title="Transactions"
        description="Manage and view all your financial transactions"
      />
      
      {/* Transactions content will go here */}
      <div className="p-8 rounded-xl border border-[var(--color-gray-border)] bg-white">
        <h3 className="text-lg font-semibold mb-4">All Transactions</h3>
        <p className="text-gray-500">
          This is where the transactions list and filters will be implemented.
        </p>
      </div>
    </div>
  );
}
